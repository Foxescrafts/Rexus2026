...
module.exports = {
    command: '/filteradd',
    aliases: ['/фильтрдобавить'],
    description: 'Добавить запрещённое слово',
    async execute(context) {
        ...
        const hasPermission = await checkCommandPriority(peerId, senderId, '/filteradd');
        if (!hasPermission) {
            const priorities = await getCommandPriorities(peerId);
            const requiredRole = priorities['/filteradd'] || 60;
        ...
        if (!word) {
            return context.reply('❌ Использование: /filteradd [слово или фраза]');
        }
        ...
        if (!filters.find(f => f.word === word)) {
            filters.push({ word, type: 1 });
            fs.writeFileSync(file, JSON.stringify(filters, null, 2));
            context.send(`✅ Запрещённое слово "${word}" добавлено.`);
        } else {
            context.send(`❌ Слово "${word}" уже в списке.`);
        }
    }
};
